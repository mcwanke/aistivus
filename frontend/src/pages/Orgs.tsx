...
function OrgRow({ org, onSelect }: { org: Org; onSelect: () => void }): React.JSX.Element {
  return (
    <button
      onClick={onSelect}
      className="w-full text-left border-b border-surface2 hover:bg-surface2 transition-colors px-[5%] py-3"
    >
  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      <div className="flex-1 overflow-y-auto">
        {isLoading && <p className="text-muted text-sm px-[5%] py-4">Loading organizations…</p>}
        {isError && <p className="text-red text-sm px-[5%] py-4">Failed to load organizations.</p>}
        {!isLoading && !isError && orgs?.length === 0 && (
          <p className="text-muted text-sm px-[5%] py-4">No organizations yet.</p>
        )}
        {orgs && orgs.map((org) => (
