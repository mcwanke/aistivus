...
interface CoverLetterSubpageProps {
  applicationId: number
  typstAvailable: boolean
}

export function CoverLetterSubpage({ applicationId, typstAvailable }: CoverLetterSubpageProps): React.JSX.Element {
  ...
  const coverDocs = documents.filter((d) => d.type_value === 'cover_letter')
  ...
  async function handleUpload(e: React.FormEvent<HTMLFormElement>): Promise<void> {
    ...
    setUploadError('')
    try {
      await upload.mutateAsync({ file: selectedFile, doc_type: 'cover_letter' })
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        {docsLoading ? (
          ...
          <p className="text-sm text-muted italic">No cover letter documents yet. Upload a file above.</p>
        ) : (
          <div>
            {coverDocs.map((doc) => (
              <DocRow
