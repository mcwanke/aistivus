...
describe('ResumeSubpage', () => {
  ...
  it('shows resume documents from default handler', async () => {
    ...
    // Query within the table to avoid matching the filename in both Evaluation Scores and Documents sections
    await waitFor(() => {
      const table = screen.getByRole('table')
      expect(within(table).getByText('resume_v1.typ')).toBeInTheDocument()
    })
  })
