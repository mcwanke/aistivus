...
COVER_PROMPT_TEMPLATE = """[[EDITABLE]]
## CONTEXT FILES
...
class ImportEvaluationRequest(BaseModel):
    ...
    score_ats: float | None = None
    score_recruiter_fast: float | None = None
    score_recruiter_deep: float | None = None
    score_candidate_role: float | None = None
    score_candidate_scope: float | None = None
    score_candidate_culture: float | None = None
...
@app.post("/api/v1/prompts/{key}/reload-from-file")
@limiter.limit("10/minute")
async def reload_prompt_from_file(request: Request, key: str):
    """
    Re-load the prompt for the given key from its source .md file in templates/prompts/.
    Inserts a new version into the DB, preserving version history.
    ...
    new_version = database.get_active_prompt(key)["version"]
    log.info("prompt_reloaded_from_file", extra={"key": key, "version": new_version})
